...
module.exports.list = function(req, res, params, cb) {
    console.log(JSON.stringify(params.body, null, 2));
    var populate = '';
    params.model.find(params.condition).sort('created_at DESC').populateAll().then(function(data) {
        if (_.isFunction(cb))
            cb(data);
        else
            res.ok(data);
    ...
    module.exports.delete = function(req, res, params) {
        console.log("Delete params Condition: " + JSON.stringify(params.condition, null, 2));

        this.list(req, res, params, function(result) {
            console.log(result);
            if (!_.isEmpty(result))
                params.model.destroy(params.condition).exec(function(err, data) {
                    res.ok();
